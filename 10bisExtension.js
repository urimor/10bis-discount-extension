...
function getDiscount() {
	const discountDiv = document.getElementsByClassName('discountCouponImgHeaderRed SpecialDiscountPercent')[0];
	if (discountDiv) {
...
function updatePrices(discount) {
	const priceDivs = document.getElementsByClassName('dishPriceDiv');
	for (let priceDiv of priceDivs) {
		priceDiv.innerHTML = priceDiv.innerHTML.replace(/₪ (\d+.\d+)/, (match, price) => {
			const newPrice = getNewPrice(price, discount).toFixed(2);
			const newPriceString = match.replace(/\d+.\d+/, newPrice);
